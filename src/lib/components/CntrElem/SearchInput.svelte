<script>
    ...
    /**
     * Создает замораживающую функцию, которая предотвращает слишком частый вызов заданной функции.
     * Замораживающая функция будет вызывать оригинальную функцию только раз в `freeze` миллисекунд.
     *
     * @param {Function} f - Функция, которую нужно заморозить
     * @returns {Function} - Замораживающая функция
     */
    function makeFreezingFunction(f){
        const freeze = 200;
        let freezed = false;
    ...
    const freezeSearch = makeFreezingFunction(search);
...
<div>
    <input type="text" placeholder="Найти блок по имени" name="searchInput" on:input={freezeSearch}>
